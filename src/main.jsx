...
function Admin({user,setPage}){
  ...
  const [analysisChart,setAnalysisChart]=useState(null),[analysisPreview,setAnalysisPreview]=useState(''),[customPips,setCustomPips]=useState({}),[customRRR,setCustomRRR]=useState({})
...
${t.notes}`,sendTelegram:true})
  ...
  const filteredAdminTrades = trades.filter(t=>adminSearchMatch(t,adminTradeSearch,['pair','category','direction','entry','stopLoss','takeProfit1','takeProfit2','riskReward','notes']) && (adminTradeFilter==='all' || signalStatusLabel(t.status,t.resultPips).toLowerCase().replace(/\s+/g,'-').includes(adminTradeFilter)))
  ...
  function tradeUpdatePayload(id,status,pips=null,rrr=''){
  ...
  async function updateTrade(id, pips, rrr=''){
    try{
      await api(`/api/admin/trades/${id}`,{method:'PUT',body:JSON.stringify(tradeUpdatePayload(id,tradeStatusFromPips(pips),pips,rrr))})
      setMsg('Trade result updated with exact pips and R:R.')
      await loadAdmin()
    }catch(err){ setMsg(err.message) }
  }
  async function setTradeStatus(id, status, resultPips=null, rrr=''){
    try{
      const payload=tradeUpdatePayload(id,status,resultPips,rrr)
