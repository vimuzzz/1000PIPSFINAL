...
function tradeFilterMatch(trade, filter){
  ...
  if(key==='cancelled') return statusLabel==='order-cancelled'
  return true
}
...
function rawTradeNumber(trade){ return trade?.tradeNumber || trade?.tradeNo || trade?.signalNo || trade?.tradeId || '' }
function tradeNumberDisplay(trade, allTrades=[]){
  const existing=rawTradeNumber(trade)
  if(existing) return String(existing)
  if(Array.isArray(allTrades) && allTrades.length && trade?._id){
    const sorted=[...allTrades].sort((a,b)=>new Date(a.createdAt||0)-new Date(b.createdAt||0))
    const idx=sorted.findIndex(t=>t._id===trade._id)
    if(idx>=0) return String(idx+1)
  }
  return ''
}
function tradeNoText(trade, allTrades=[]){ const n=tradeNumberDisplay(trade,allTrades); return n?`Trade No ${n}`:'Trade No' }
function notesWithTradeNumber(notes, tradeNo){
  const clean=String(notes||'').trim()
  if(!tradeNo) return clean
  if(/trade\s*(no|#|number)/i.test(clean)) return clean
  return `Trade No ${tradeNo}${clean?`\n${clean}`:''}`
}
function formatPercent(value){ const n=round2(value); if(!Number.isFinite(n)||n===0) return '0%'; return `${n>0?'+':''}${n}%` }
...
function buildWeeklyReportText(report,trades=[]){
  ...
  if(highlights.length){
    ...
    highlights.forEach(t=>{
      ...
      lines.push(`• ${tradeNoText(t,trades)}: ${t.pair} ${t.direction} - ${signalStatusLabel(t.status,t.resultPips)} (${p>0?'+':''}${p} pips${optionalPercent(t.resultPercent)?`, ${optionalPercent(t.resultPercent)}`:''})`)
    })
  }
...
function downloadWeeklyPerformancePoster(report,trades=[],format='portrait'){
  ...
  const presets={
    ...
    status:{width:1080,height:1920,label:'whatsapp-status',highlights:4,summaryLines:7,cardH:122,statFont:40}
  }
  ...
  statItems.forEach((item,i)=>{
    ...
    ctx.font='24px Arial'
    ctx.fillText(item[0],x+22,cy+34)
    ctx.fillStyle='#ffffff'
    ctx.font=`700 ${preset.statFont}px Arial`
    ctx.fillText(String(item[1]),x+22,cy+78)
  })
  const statRows=Math.ceil(statItems.length/2)
  y += (cardH*statRows) + (gap*(statRows-1)) + 28

  const updateBoxH = format==='status' ? 260 : format==='portrait' ? 205 : 180
  ctx.fillStyle='rgba(255,255,255,0.06)'
  ...
  if(!highlights.length){
    ...
    highlights.forEach((t,idx)=>{
      const lineY=y+86+(idx*38)
      const p=Number(t.resultPips||0)
      ctx.fillStyle='#ffffff'
      ctx.font='700 24px Arial'
      ctx.fillText(`${tradeNoText(t,trades)} · ${t.pair} ${t.direction}`,margin+28,lineY)
      ctx.fillStyle='#aab3c5'
      ctx.font='22px Arial'
      ...
      ctx.textAlign='right'
      const resultText = `${p>0?'+':''}${p} pips${optionalPercent(t.resultPercent)?' '+optionalPercent(t.resultPercent):''}`
      ctx.fillText(resultText,W-margin-30,lineY)
      ctx.textAlign='left'
    })
  }
  y += updateBoxH + 22
  ...
  const footerH=70
  const summaryBoxH = Math.max(170, H - y - footerH - 24)
  ctx.fillStyle='rgba(255,255,255,0.06)'
  ...
  ctx.font='25px Arial'
  summaryLines.forEach((line,i)=>ctx.fillText(line, margin+28, y+88+(i*34)))
...
function TradeCard({trade, allTrades=[]}){
  ...
  const tradeNo=tradeNumberDisplay(trade,allTrades)
    ...
    <div className="premiumSignalTop">
      <div>
        <div className="premiumSignalPair">
          ...
          {tradeNo&&<span className="tradeNoBadge">Trade No {tradeNo}</span>}
          <h3>{trade.pair}</h3>
...
function SignalDashboard({user,setPage}){
  ...
  return <section className="section">
    ...
    <div className="listGrid">{filteredTrades.map(t=><TradeCard key={t._id} trade={t} allTrades={trades}/>)}{trades.length===0&&<p>No trades yet.</p>}{trades.length>0&&weeklyTrades.length===0&&<p>No current or last week trades yet.</p>}{weeklyTrades.length>0&&filteredTrades.length===0&&<p>No signals found for this filter.</p>}</div>
  </section>
}
...
function Admin({user,setPage}){
  ...
  const [trade,setTrade]=useState({tradeNumber:'',pair:'XAUUSD',category:'Gold',direction:'BUY',entry:'3350',stopLoss:'3340',takeProfit1:'3370',takeProfit2:'3385',riskReward:'1:2',riskPercent:'1',resultPercent:'',notes:'Trend continuation setup',sendTelegram:true})
  ...
  function useTradeTemplate(t){
    const next={...trade,tradeNumber:trade.tradeNumber||'',pair:t.pair,category:t.category,direction:t.direction,entry:'',stopLoss:'',takeProfit1:'',takeProfit2:'',riskReward:'1:2',riskPercent:trade.riskPercent||'1',resultPercent:'',notes:t.notes,sendTelegram:true}
    setTrade(next)
...
${t.notes}`,sendTelegram:true})
  ...
  const filteredAdminTrades = trades.filter(t=>adminSearchMatch(t,adminTradeSearch,['tradeNumber','tradeNo','signalNo','pair','category','direction','entry','stopLoss','takeProfit1','takeProfit2','riskReward','riskPercent','resultPercent','notes']) && (adminTradeFilter==='all' || signalStatusLabel(t.status,t.resultPips).toLowerCase().replace(/\s+/g,'-').includes(adminTradeFilter)))
  ...
  function nextTradeNumber(){
    const nums=trades.map(t=>Number(rawTradeNumber(t))).filter(n=>Number.isFinite(n)&&n>0)
    return String(nums.length?Math.max(...nums)+1:trades.length+1)
  }
  function preparedTradePayload(){
    const no=String(trade.tradeNumber||nextTradeNumber()).trim()
    return {...trade, tradeNumber:no, tradeNo:no, signalNo:no, notes:notesWithTradeNumber(trade.notes,no)}
  }
  async function addTrade(e){ e.preventDefault(); try{ await api('/api/admin/trades',{method:'POST',body:JSON.stringify(preparedTradePayload())}); setMsg(`Trade No ${trade.tradeNumber||nextTradeNumber()} signal added successfully.`); setTrade({...trade,tradeNumber:''}); await loadAdmin() }catch(err){ setMsg(err.message) } }
  function tradeUpdatePayload(id,status,pips=null,rrr='',resultPercent=''){
    const currentTrade=trades.find(t=>t._id===id) || {}
    const no=tradeNumberDisplay(currentTrade,trades)
    const payload={status}
    if(no){ payload.tradeNumber=no; payload.tradeNo=no; payload.signalNo=no; payload.telegramPrefix=`Trade No ${no}` }
  ...
  function startEditTrade(t){ setEditingTrade({ _id:t._id, tradeNumber:tradeNumberDisplay(t,trades), pair:t.pair||'', category:t.category||'Forex', direction:t.direction||'BUY', entry:t.entry||'', stopLoss:t.stopLoss||'', takeProfit1:t.takeProfit1||'', takeProfit2:t.takeProfit2||'', riskReward:t.riskReward||'', riskPercent:t.riskPercent||'', resultPercent:t.resultPercent||'', notes:t.notes||'', status:t.status||'active', resultPips:t.resultPips||0 }) }
  async function saveTradeEdit(e){ e.preventDefault(); if(!editingTrade?._id) return; try{ const {_id,...payload}=editingTrade; payload.resultPips=Number(payload.resultPips||0); payload.riskPercent=Number(payload.riskPercent||0); payload.resultPercent=Number(payload.resultPercent||0); payload.tradeNo=payload.tradeNumber; payload.signalNo=payload.tradeNumber; payload.notes=notesWithTradeNumber(payload.notes,payload.tradeNumber); await api(`/api/admin/trades/${_id}`,{method:'PUT',body:JSON.stringify(payload)}); setEditingTrade(null); setMsg('Trade signal edited successfully.'); await loadAdmin() }catch(err){ setMsg(err.message) } }
  ...
  return <section className="section"><p className="green">ADMIN DASHBOARD</p><h2>Telegram Analysis Image Posting + Full Management</h2><button className="refresh" onClick={loadAdmin}>Refresh Admin Data</button>{msg&&<p className={msg.toLowerCase().includes('success')||msg.toLowerCase().includes('refreshed')||msg.toLowerCase().includes('saved')||msg.toLowerCase().includes('sent')||msg.toLowerCase().includes('posted')?'success':'error'}>{msg}</p>}{viewer&&<div className="modal" onClick={()=>setViewer(null)}><div className="modalInner"><button onClick={()=>setViewer(null)}>Close</button><img src={viewer}/></div></div>}
    ...
    <div className="adminGrid">
      ...
      <div className="adminBox"><h3>Add Trade Signal</h3>
        ...
        <form onSubmit={addTrade} className="form compact"><input placeholder="Trade No e.g. 1 (leave empty for auto)" value={trade.tradeNumber} onChange={e=>setTrade({...trade,tradeNumber:e.target.value})}/><input placeholder="Pair" value={trade.pair} onChange={e=>setTrade({...trade,pair:e.target.value})}/><select value={trade.category} onChange={e=>setTrade({...trade,category:e.target.value})}><option>Gold</option><option>Forex</option><option>Crypto</option><option>Indices</option><option>Oil</option></select><select value={trade.direction} onChange={e=>setTrade({...trade,direction:e.target.value})}><option>BUY</option><option>SELL</option></select><input placeholder="Entry" value={trade.entry} onChange={e=>setTrade({...trade,entry:e.target.value})}/><input placeholder="Stop Loss" value={trade.stopLoss} onChange={e=>setTrade({...trade,stopLoss:e.target.value})}/><input placeholder="TP1" value={trade.takeProfit1} onChange={e=>setTrade({...trade,takeProfit1:e.target.value})}/><input placeholder="TP2" value={trade.takeProfit2} onChange={e=>setTrade({...trade,takeProfit2:e.target.value})}/><input placeholder="Risk Reward e.g. 1:2" value={trade.riskReward} onChange={e=>setTrade({...trade,riskReward:e.target.value})}/><input type="number" step="0.01" placeholder="Risk % of capital e.g. 1" value={trade.riskPercent} onChange={e=>setTrade({...trade,riskPercent:e.target.value})}/><input type="number" step="0.01" placeholder="Expected result % optional e.g. 2" value={trade.resultPercent} onChange={e=>setTrade({...trade,resultPercent:e.target.value})}/><textarea placeholder="Notes" value={trade.notes} onChange={e=>setTrade({...trade,notes:e.target.value})}/><label className="check"><input type="checkbox" checked={trade.sendTelegram} onChange={e=>setTrade({...trade,sendTelegram:e.target.checked})}/> Send to Telegram</label><button>Add Trade</button></form></div>
      <div className="adminBox full"><h3>Manage Trades With Exact Pips</h3><AdminFilterBar search={adminTradeSearch} setSearch={setAdminTradeSearch} filter={adminTradeFilter} setFilter={setAdminTradeFilter} placeholder="Search pair, market, notes..." total={trades.length} shown={filteredAdminTrades.length} filters={[['all','All Status'],['running','Running'],['tp1-hit','TP1 Hit'],['tp2-hit','TP2 Hit'],['sl-hit','SL Hit'],['break-even','Break Even'],['closed','Closed'],['cancelled','Cancelled']]}/>{filteredAdminTrades.length===0&&<p>No trades found.</p>}<AdminShowToggle itemsKey="trades" total={filteredAdminTrades.length}/>{adminItems(filteredAdminTrades,'trades').map(t=>{ const statusLabel=signalStatusLabel(t.status,t.resultPips); const statusClass=signalStatusClass(t.status,t.resultPips); const tradeNo=tradeNumberDisplay(t,trades); return <div className="adminRow" key={t._id}><strong>{tradeNo?`Trade No ${tradeNo} · `:''}{t.pair} {t.direction}</strong><p>Status: <span className={`signalStatus ${statusClass}`}>{statusLabel}</span> | Current pips: {t.resultPips} | R:R: {t.riskReward || 'N/A'} | Risk: {Number(t.riskPercent||0)}% | Result: {formatPercent(t.resultPercent)}</p><p className="signalDate">Signal Given: {formatDateTime(t.createdAt)}</p><div className="exactPipsRow tradeResultInputs"><input type="number" placeholder="Exact pips e.g. 145 / -50" value={customPips[t._id]||''} onChange={e=>setCustomPips({...customPips,[t._id]:e.target.value})}/><input placeholder="R:R e.g. 1:2.5" value={customRRR[t._id]||''} onChange={e=>setCustomRRR({...customRRR,[t._id]:e.target.value})}/><input type="number" step="0.01" placeholder="Gain/Loss % e.g. 2 / -1" value={customResultPercent[t._id]||''} onChange={e=>setCustomResultPercent({...customResultPercent,[t._id]:e.target.value})}/></div><div className="rowBtns tradeUpdateBtns"><button onClick={()=>setTradeStatus(t._id,'active',0,customRRR[t._id]||t.riskReward||'',0)}>Running</button><button onClick={()=>setTradeStatus(t._id,'tp1',customPips[t._id]||t.resultPips||0,customRRR[t._id]||t.riskReward||'',customResultPercent[t._id]||t.resultPercent||'')}>TP1 Hit + Pips</button><button onClick={()=>setTradeStatus(t._id,'tp2',customPips[t._id]||t.resultPips||0,customRRR[t._id]||t.riskReward||'',customResultPercent[t._id]||t.resultPercent||'')}>TP2 Hit + Pips</button><button onClick={()=>setTradeStatus(t._id,'sl',customPips[t._id]||t.resultPips||0,customRRR[t._id]||t.riskReward||'',customResultPercent[t._id]||t.resultPercent||'')}>SL Hit + Pips</button><button onClick={()=>setTradeStatus(t._id,'breakeven',0,customRRR[t._id]||t.riskReward||'',0)}>BE 0</button><button onClick={()=>updateTrade(t._id, customPips[t._id]||0, customRRR[t._id]||t.riskReward||'', customResultPercent[t._id]||t.resultPercent||'')}>Manual Close + Pips</button><button className="cancelOrderBtn" onClick={()=>setTradeStatus(t._id,'cancelled',0,customRRR[t._id]||t.riskReward||'',0)}>Cancel Order</button><button onClick={()=>startEditTrade(t)}>Edit</button><button onClick={()=>deleteTrade(t._id)}>Hide from Dashboard</button></div>{editingTrade?._id===t._id&&<form onSubmit={saveTradeEdit} className="form compact"><input placeholder="Trade No" value={editingTrade.tradeNumber||''} onChange={e=>setEditingTrade({...editingTrade,tradeNumber:e.target.value})}/><input placeholder="Pair" value={editingTrade.pair} onChange={e=>setEditingTrade({...editingTrade,pair:e.target.value})}/><select value={editingTrade.category} onChange={e=>setEditingTrade({...editingTrade,category:e.target.value})}><option>Gold</option><option>Forex</option><option>Crypto</option><option>Indices</option><option>Oil</option></select><select value={editingTrade.direction} onChange={e=>setEditingTrade({...editingTrade,direction:e.target.value})}><option>BUY</option><option>SELL</option></select><input placeholder="Entry" value={editingTrade.entry} onChange={e=>setEditingTrade({...editingTrade,entry:e.target.value})}/><input placeholder="Stop Loss" value={editingTrade.stopLoss} onChange={e=>setEditingTrade({...editingTrade,stopLoss:e.target.value})}/><input placeholder="TP1" value={editingTrade.takeProfit1} onChange={e=>setEditingTrade({...editingTrade,takeProfit1:e.target.value})}/><input placeholder="TP2" value={editingTrade.takeProfit2} onChange={e=>setEditingTrade({...editingTrade,takeProfit2:e.target.value})}/><input placeholder="Risk Reward" value={editingTrade.riskReward} onChange={e=>setEditingTrade({...editingTrade,riskReward:e.target.value})}/><input type="number" step="0.01" placeholder="Risk %" value={editingTrade.riskPercent} onChange={e=>setEditingTrade({...editingTrade,riskPercent:e.target.value})}/><input type="number" step="0.01" placeholder="Result %" value={editingTrade.resultPercent} onChange={e=>setEditingTrade({...editingTrade,resultPercent:e.target.value})}/><select value={editingTrade.status} onChange={e=>setEditingTrade({...editingTrade,status:e.target.value})}><option value="active">Running</option><option value="tp1">TP1 Hit</option><option value="tp2">TP2 Hit</option><option value="sl">SL Hit</option><option value="breakeven">Break Even</option><option value="closed">Closed</option><option value="cancelled">Order Cancelled</option></select><input type="number" placeholder="Result pips" value={editingTrade.resultPips} onChange={e=>setEditingTrade({...editingTrade,resultPips:e.target.value})}/><textarea placeholder="Notes" value={editingTrade.notes} onChange={e=>setEditingTrade({...editingTrade,notes:e.target.value})}/><div className="rowBtns"><button>Save Edit</button><button type="button" onClick={()=>setEditingTrade(null)}>Cancel</button></div></form>}</div>})}</div>
