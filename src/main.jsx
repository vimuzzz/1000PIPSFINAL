...
function signalStatusLabel(status, resultPips){
  ...
  if(['tp','sl','breakeven','closed'].includes(s)) return 'CLOSED'
  if(Number(resultPips)!==0) return 'CLOSED'
  return 'RUNNING'
}
function signalStatusClass(status, resultPips){ return signalStatusLabel(status,resultPips).toLowerCase() }
...
function TradeCard({trade}){
  ...
  return <div className={`tradeCard ${trade.status} ${statusClass}`}>
    <div>
      <h3>{trade.pair} <span>{trade.direction}</span></h3>
      <p>{trade.category} · <span className={`signalStatus ${statusClass}`}>{statusLabel}</span></p>
      <p className="signalDate">Signal Given: {formatDateTime(trade.createdAt)}</p>
    </div>
    <div className="tradeLevels">
      <p>Entry: {trade.entry}</p>
      <p>SL: {trade.stopLoss}</p>
      <p>TP1: {trade.takeProfit1}</p>
      <p>TP2: {trade.takeProfit2}</p>
      <p>Pips: {trade.resultPips}</p>
    </div>
    {trade.notes&&<pre>{trade.notes}</pre>}
  </div>
}
...
function Admin({user,setPage}){
  ...
  async function deleteTrade(id){ if(!window.confirm('Delete this trade signal permanently?')) return; try{ await api(`/api/admin/trades/${id}`,{method:'DELETE'}); setMsg('Trade signal deleted successfully.'); await loadAdmin() }catch(err){ setMsg(err.message) } }
  ...
  return <section className="section"><p className="green">ADMIN DASHBOARD</p><h2>Telegram Analysis Image Posting + Full Management</h2><button className="refresh" onClick={loadAdmin}>Refresh Admin Data</button>{msg&&<p className={msg.toLowerCase().includes('success')||msg.toLowerCase().includes('refreshed')||msg.toLowerCase().includes('saved')||msg.toLowerCase().includes('sent')||msg.toLowerCase().includes('posted')?'success':'error'}>{msg}</p>}{viewer&&<div className="modal" onClick={()=>setViewer(null)}><div className="modalInner"><button onClick={()=>setViewer(null)}>Close</button><img src={viewer}/></div></div>}
    ...
    <div className="adminGrid">
      ...
      <div className="adminBox full"><h3>Manage Trades With Exact Pips</h3>{trades.length===0&&<p>No trades found.</p>}{trades.map(t=>{ const statusLabel=signalStatusLabel(t.status,t.resultPips); const statusClass=signalStatusClass(t.status,t.resultPips); return <div className="adminRow" key={t._id}><strong>{t.pair} {t.direction}</strong><p>Status: <span className={`signalStatus ${statusClass}`}>{statusLabel}</span> | Current pips: {t.resultPips}</p><p className="signalDate">Signal Given: {formatDateTime(t.createdAt)}</p><div className="rowBtns"><button onClick={()=>updateTrade(t._id,100)}>TP +100</button><button onClick={()=>updateTrade(t._id,-50)}>SL -50</button><button onClick={()=>updateTrade(t._id,0)}>BE 0</button><button onClick={()=>startEditTrade(t)}>Edit</button><button onClick={()=>deleteTrade(t._id)}>Delete</button></div><div className="exactPipsRow"><input type="number" placeholder="Exact pips" value={customPips[t._id]||''} onChange={e=>setCustomPips({...customPips,[t._id]:e.target.value})}/><button onClick={()=>updateTrade(t._id, customPips[t._id]||0)}>Close With Exact Pips</button></div>{editingTrade?._id===t._id&&<form onSubmit={saveTradeEdit} className="form compact"><input placeholder="Pair" value={editingTrade.pair} onChange={e=>setEditingTrade({...editingTrade,pair:e.target.value})}/><select value={editingTrade.category} onChange={e=>setEditingTrade({...editingTrade,category:e.target.value})}><option>Gold</option><option>Forex</option><option>Crypto</option><option>Indices</option><option>Oil</option></select><select value={editingTrade.direction} onChange={e=>setEditingTrade({...editingTrade,direction:e.target.value})}><option>BUY</option><option>SELL</option></select><input placeholder="Entry" value={editingTrade.entry} onChange={e=>setEditingTrade({...editingTrade,entry:e.target.value})}/><input placeholder="Stop Loss" value={editingTrade.stopLoss} onChange={e=>setEditingTrade({...editingTrade,stopLoss:e.target.value})}/><input placeholder="TP1" value={editingTrade.takeProfit1} onChange={e=>setEditingTrade({...editingTrade,takeProfit1:e.target.value})}/><input placeholder="TP2" value={editingTrade.takeProfit2} onChange={e=>setEditingTrade({...editingTrade,takeProfit2:e.target.value})}/><input placeholder="Risk Reward" value={editingTrade.riskReward} onChange={e=>setEditingTrade({...editingTrade,riskReward:e.target.value})}/><select value={editingTrade.status} onChange={e=>setEditingTrade({...editingTrade,status:e.target.value})}><option value="active">Running</option><option value="tp">Closed TP</option><option value="sl">Closed SL</option><option value="breakeven">Closed BE</option><option value="closed">Closed</option></select><input type="number" placeholder="Result pips" value={editingTrade.resultPips} onChange={e=>setEditingTrade({...editingTrade,resultPips:e.target.value})}/><textarea placeholder="Notes" value={editingTrade.notes} onChange={e=>setEditingTrade({...editingTrade,notes:e.target.value})}/><div className="rowBtns"><button>Save Edit</button><button type="button" onClick={()=>setEditingTrade(null)}>Cancel</button></div></form>}</div>})}</div>
