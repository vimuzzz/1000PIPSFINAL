...
function Admin({user,setPage}){
  ...
  async function loadAdmin(){ setMsg(''); try{ const [u,p,rpt,rep,an,tr,refs,pr]=await Promise.all([api('/api/admin/users'),api('/api/admin/payments'),api('/api/admin/report'),api('/api/vip/reports'),api('/api/vip/analysis'),api('/api/vip/trades'),api('/api/vip/proofs')]); setUsers(u); setPayments(p); setReport(rpt); setReports(rep); setAnalysis(an); setTrades(tr); setAdminReferrals(refs); setProofs(pr); setMsg('Admin data refreshed.') }catch(e){ setMsg(e.message) } }
  ...
  return <section className="section"><p className="green">ADMIN DASHBOARD</p><h2>Telegram Analysis Image Posting + Full Management</h2><button className="refresh" onClick={loadAdmin}>Refresh Admin Data</button>{msg&&<p className={msg.toLowerCase().includes('success')||msg.toLowerCase().includes('refreshed')||msg.toLowerCase().includes('saved')||msg.toLowerCase().includes('sent')||msg.toLowerCase().includes('posted')?'success':'error'}>{msg}</p>}{viewer&&<div className="modal" onClick={()=>setViewer(null)}><div className="modalInner"><button onClick={()=>setViewer(null)}>Close</button><img src={viewer}/></div></div>}
    ...
    <div className="adminGrid">
      ...
      <div className="adminBox full"><h3>Referral / Affiliate Tracking</h3>
        ...
        {adminReferrals.map(r=><div key={r._id} className="adminRow">
          <strong>{r.referredName || r.referredEmail}</strong>
          <p>Referred by: {r.referrerEmail}</p>
          <p>Status: {r.status} | Plan: {r.plan || 'Not selected yet'}</p>
          <div className="rowBtns"><button onClick={()=>updateReferral(r._id,'pending')}>Pending</button><button onClick={()=>updateReferral(r._id,'approved')}>Approved</button><button onClick={()=>updateReferral(r._id,'paid')}>Paid</button></div>
        </div>)}
      </div>
